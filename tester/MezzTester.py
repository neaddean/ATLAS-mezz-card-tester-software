...
cur.execute("CREATE TABLE IF NOT EXISTS cards "
            "(rowid INTEGER PRIMARY KEY AUTOINCREMENT, MezzID INT, asd1_id INT, "
            "asd2_id INT, asd3_id INT, user TEXT, loc TEXT, pass TEXT, comments TEXT,"
            "date TEXT default current_timestamp, analog_voltage INT, "
            "digital_voltage INT, analog_current INT, digital_current INT, "
            "temperature INT, tdc_jtag_test TEXT, tdc_id_test TEXT,"
            "asd_jtag_test TEXT, asd_tdc TEXT, strobe_test TEXT, trig_test TEXT);")
cur.execute("CREATE TABLE IF NOT EXISTS channels (MezzID INT, Chan INT, Ro INT, Voff INT,"
            "sigma INT, m_fit INT, b_fit INT, r INT, std_err INT);")
cur.execute("CREATE TABLE IF NOT EXISTS sweep_data (MezzID INT, cards_id INT,"
            "Chan INT, thresh INT, hits INT, freq INT, window INT, error INT);")
cur.execute("CREATE TABLE IF NOT EXISTS dac_data "
            "(MezzID INT, cards_id INT, Chan INT, thresh INT, dac INT);")
...
while (running):
    ...
    if mezzID == "q":
        break
    asd1  = raw_input("Please enter serial number of asd 1 (q to quit): ")
    if mezzID == "q":
        break
    asd2  = raw_input("Please enter serial number of asd 2 (q to quit): ")
    if mezzID == "q":
        break
    asd3  = raw_input("Please enter serial number of asd 3 (q to quit): ")
    if mezzID == "q":
        break
    ...
    if len(rows) > 0:
        rows.sort(key=lambda rows: datetime.datetime.strptime(
            rows["date"], '%Y-%m-%d %H:%M:%S'))
        print "This card was most recently tested by %s at %s on %s (utc)" % (
            rows[-1]["user"], rows[-1]["loc"], rows[-1]["date"])
        print "Result: "
    ...
    cur.execute("INSERT INTO cards (MezzID, user, loc, pass, comments, analog_voltage,"
                "digital_voltage, analog_current, digital_current, temperature, tdc_jtag_test,"
                "tdc_id_test, asd_jtag_test, asd_tdc, strobe_test, trig_test) "
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                (mezzID, user, loc, "pass", "none", adclog["AV"], adclog["DV"],
                 adclog["AC"], adclog["DC"], adclog["TEMP"], card_data_list[0],
                 card_data_list[1], card_data_list[2], card_data_list[3],
                 card_data_list[4], card_data_list[5]))
    card_rowid = cur.lastrowid

    channel_params_list = []
    for x in range(len(sweep_params)):
        channel_params_list.append((mezzID, x, sweep_params[x][0],
                                  sweep_params[x][1],  sweep_params[x][2],
                                  dac_params[x][0], dac_params[x][1],
                                  dac_params[x][2], dac_params[x][3]))
    sweep_data_list = []
    for x in range(len(sweep_data)):
        sweep_data[x] = np.transpose(sweep_data[x])
        for y in range(len(sweep_data[x])):
            sweep_data_list.append((mezzID, card_rowid, x, sweep_data[x][y][0],
                                    sweep_data[x][y][1], sweep_data[x][y][2],
                                    sweep_data[x][y][3], sweep_data[x][y][4]))
    dac_data_list = []
    for x in range(len(dac_data)):
       dac_data[x] = np.transpose(dac_data[x])
       for y in range(len(dac_data[x])):
           dac_data_list.append((mezzID, card_rowid, x, dac_data[x][y][0],  dac_data[x][y][1]))


    cur.executemany("INSERT INTO channels (MezzID, Chan, Ro, Voff, sigma,"
                    "m_fit, b_fit, r, std_err)"
                    "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)", channel_params_list)
        
    cur.executemany("INSERT INTO sweep_data (MezzID, cards_id, Chan, thresh, freq, hits, window,"
                    "error) VALUES (?, ?, ?, ?, ?, ?, ?, ?)", sweep_data_list)

    cur.executemany("INSERT INTO dac_data (MezzID, cards_id, Chan, thresh, dac)"
    ...
    for result in testresults:
        if result["result"] == "pass":
            continue
        passed = False
        print "ERROR: test %s failed with comments: %s" % (
            result["test"], ", ".join(result["comments"]))
