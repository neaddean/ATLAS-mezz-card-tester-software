...
cur.execute("CREATE TABLE IF NOT EXISTS cards "
            "(rowid INTEGER PRIMARY KEY AUTOINCREMENT, MezzID INT, user TEXT, loc TEXT, pass TEXT, comments TEXT,"
            "date TEXT default current_timestamp, tdc_jtag_test TEXT, tdc_id_test TEXT,"
...
cur.execute("CREATE TABLE IF NOT EXISTS sweep_data (MezzID INT, Chan INT, thresh INT,"
            "hits INT, freq INT, window INT, error INT);")
...
while (running):
    ...
    for x in range(len(sweep_params)):
        sweep_params_list.append((mezzID, x, sweep_params[x][0],
    ...
    for x in range(len(sweep_data)):
        ...
        for y in range(len(sweep_data[x])):
            sweep_data_list.append((mezzID, x, sweep_data[x][y][0],
    ...
    cur.execute("INSERT INTO cards (MezzID, user, loc, pass, comments, tdc_jtag_test, "
                "tdc_id_test, asd_jtag_test, asd_tdc, strobe_test, trig_test) "
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                (mezzID, user, loc, "pass", "none", card_data_list[0],
    ...
    print cur.lastrowid
    cur.executemany("INSERT INTO sweep_data (MezzID, Chan, thresh, freq, hits, window,"
